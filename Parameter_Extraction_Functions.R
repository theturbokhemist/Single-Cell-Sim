...
identify_reference_models <- function(target_expression, reference_object, nth_neighbor = 20, cutoff_percent = 0.85, n_closest_neighbors = NULL) {
  ...
  density.ref.target.plot <- ggplot() + geom_density(data = as.data.frame(target_expression), aes(A, color = "A"), linetype = "dashed") +
    ...
    labs(title="Expression Density")  +  theme(plot.title = element_text(hjust = 0.5)) +   scale_colour_manual("Gene", values = c("blue", "red", "blue", "red"),
    labels = c(A = "Target(A)", B = "Target(B)", C = "Reference(A)", D = "Reference(B)"))
  ...
  density.kept_ref.target.plot <- ggplot() + geom_density(data = as.data.frame(target_expression), aes(A, color = "A"), linetype = "dashed") +
    ...
    labs(title="Expression Density")  +  theme(plot.title = element_text(hjust = 0.5)) +   scale_colour_manual("Gene", values = c("blue", "red", "blue", "red"),
    labels = c(A = "Target(A)", B = "Target(B)", C = "Kept Reference(A)", D = "Kept Reference(B)"))
  ...
  if (!is.null(reference_object$resampled_expression)) {
    ...
    total_overlay <- ggplot() + 
      ...
      geom_point(data = as.data.frame(standard_expression_TS.kmeans$centers), aes(A, B, color="DF5"), shape=13, size=7, alpha = 5) + 
      labs(title="Expression Scatterplot") +  theme(plot.title = element_text(hjust = 0.5), legend.position = c(0, 0), legend.justification = c("left", "bottom")) +
...
optimize_parameters <- function(reference_object, target_object, num_iterations = 5, nth_neighbor = 20, cutoff_percent = 0.85, nbins = 10, n_closest_neighbors = NULL, 
  ...
  knn <- knn(train = standard_expression_TS.kmeans$centers, test = reference_object$expression_ref, cl = factor(c(1,2)), k = 1)
  knn_freq <- table(knn)
  ...
  for (i in 1:num_iterations) {
    ...
    knn <- knn(train = standard_expression_TS.kmeans$centers, test = identified_ref_models$reference_expression_filtered, cl = factor(c(1,2)), k = 1)
    knn_freq <- table(knn)
    ratio <- 100*knn_freq[1]/knn_freq[2]
    ...
    knn <- knn(train = standard_expression_TS.kmeans$centers, test = reference_object$resampled_expression, cl = factor(c(1,2)), k = 1)
    knn_freq <- table(knn)
    ratio <- 100*knn_freq[1]/knn_freq[2]
  ...
  knn_target <- knn(train = standard_expression_TS.kmeans$centers, test = target_object$simulated_rset_expr_log_z, cl = factor(c(1,2)), k = 1)
